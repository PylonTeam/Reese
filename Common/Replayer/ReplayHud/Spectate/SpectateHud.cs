...
internal sealed class SpectateHud : UIElement
{
    ...
    private const float StatusTextScale = 1.2f;
    private const float StatusPanelMinHeight = 36f;
    ...
    public SpectateHud()
    {
        ...
        Top.Set(ReplayInfo.InfoHud.TopOffset, 0f);
        Width.Set(ReplayInfo.InfoHud.PanelWidth, 0f);
        Height.Set(ReplayInfo.InfoHud.PanelHeight, 0f);
    ...
    public void Rebuild()
    {
        ...
        float panelWidth = ReplayInfo.InfoHud.PanelWidth;
        float contentWidth = Math.Max(0f, panelWidth - playerPanelPadding * 2f);
        ...
        shownPlayerCards = GetShownCardsForTargetCount(playerTargets.Count, contentWidth, cardWidth, cardGap);
        shownNpcCards = GetShownCardsForTargetCount(npcTargets.Count, contentWidth, cardWidth, cardGap);
        ...
        // Layout
        float playerCardsStart = GetCardsStart(contentWidth, shownPlayerCards, cardWidth, cardGap);
        float npcCardsStart = GetCardsStart(contentWidth, shownNpcCards, cardWidth, cardGap);
        ...
        contentPanel.Height.Set(GetMinContentHeight(), 0f);
        contentPanel.BackgroundColor = UICommon.DefaultUIBlueMouseOver * 0.3f;
        contentPanel.BorderColor = Color.Black;
        Append(contentPanel);

        BuildContent(playerTargets, npcTargets, visibleTargets, visibleNpcTargets, scale, playerPanelPadding, navButtonWidth, cardGap, cardWidth, cardHeight, playerCardsStart, npcCardsStart);
        ...
        float npcCardsStart)
    {
        ...
        if (currentTab?.Tab == SpectatorTab.Players)
        {
            ...
            AddPrevButton(contentPanel, playerPanelPadding, navButtonWidth, cardHeight, scale, () => NavigateTarget(-1), "Go to previous player");
            ...
            for (int i = 0; i < visiblePlayerTargets; i++)
            {
                ...
                playerCard.Left.Set(playerCardsStart + i * (cardWidth + cardGap), 0f);
            ...
            AddNextButton(contentPanel, playerPanelPadding, navButtonWidth, cardHeight, scale, () => NavigateTarget(1), "Go to next player");
            return;
        }
        ...
        AddPrevButton(contentPanel, playerPanelPadding, navButtonWidth, cardHeight, scale, () => NavigateNpcTarget(-1), "Go to previous NPC");
        ...
        for (int i = 0; i < visibleNpcTargets; i++)
        {
            ...
            npcCard.Left.Set(npcCardsStart + i * (cardWidth + cardGap), 0f);
        ...
        AddNextButton(contentPanel, playerPanelPadding, navButtonWidth, cardHeight, scale, () => NavigateNpcTarget(1), "Go to next NPC");
    }
    ...
    private void SetPanelLeft(bool shiftedForPlayerHud)
    {
        ...
        Left.Set(-ReplayInfo.InfoHud.RightOffset - (shiftedForPlayerHud ? PlayerHudOpenOffset : 0f), 0f);
    }
    ...
    private void SetStatusTextInternal(string text)
    {
        ...
        string wrappedText = WrapStatusText(text, GetStatusTextMaxWidth(), textScale, out int lineCount);
        statusText.SetText(wrappedText, textScale, false);
        UpdateStatusPanelLayout(lineCount);
    }

    private void UpdateStatusPanelLayout(int lineCount)
    {
        float lineHeight = FontAssets.MouseText.Value.LineSpacing * GetStatusTextScale();
        float textHeight = Math.Max(lineHeight, lineCount * lineHeight);
        float padding = GetStatusPanelPadding();
        float statusHeight = Math.Max(textHeight + padding * 2f, StatusPanelMinHeight);

        float minContentHeight = GetMinContentHeight();
        float maxStatusHeight = Math.Max(0f, ReplayInfo.InfoHud.PanelHeight - GetHeaderHeight() - GetTabHeight() - ContentGap - minContentHeight);

        if (maxStatusHeight > 0f)
            statusHeight = Math.Min(statusHeight, maxStatusHeight);

        float contentHeight = Math.Max(0f, ReplayInfo.InfoHud.PanelHeight - GetHeaderHeight() - GetTabHeight() - statusHeight - ContentGap);

        contentPanel?.Height.Set(contentHeight, 0f);
        statusPanel.Height.Set(statusHeight, 0f);
        statusPanel.Top.Set(GetHeaderHeight() + GetTabHeight() + contentHeight + ContentGap, 0f);
        Recalculate();
    }
    ...
    private static float GetMinContentHeight()
    {
        return UIPlayerCard.CardHeight + GetPlayerPanelPadding() * 2f;
    }

    private static int GetShownCardsForTargetCount(int targetCount, float contentWidth, float cardWidth, float cardGap)
    {
        if (targetCount <= 0)
            return MinShownPlayerCards;

        int maxByCount = Math.Min(MaxShownPlayerCards, targetCount);
        int desired = userChangedShownPlayerCards ? requestedShownPlayerCards : targetCount;
        int maxByWidth = Math.Max(1, (int)Math.Floor((contentWidth + cardGap) / (cardWidth + cardGap)));
        int maxShown = Math.Min(maxByCount, maxByWidth);

        return Math.Clamp(desired, MinShownPlayerCards, maxShown);
    }

    private static float GetCardsStart(float contentWidth, int shownCards, float cardWidth, float cardGap)
    {
        float shownCardsWidth = shownCards * cardWidth + Math.Max(0, shownCards - 1) * cardGap;
        return Math.Max(0f, (contentWidth - shownCardsWidth) * 0.5f);
    }

    private static float GetStatusTextMaxWidth()
    {
        float statusPadding = GetStatusPanelPadding();
        float width = ReplayInfo.InfoHud.PanelWidth - statusPadding * 2f;
        return Math.Max(40f, width);
    }
    ...
    private void AddPrevButton(UIPanel playersPanel, float playerPanelPadding, float navButtonWidth, float cardHeight, float scale, Action onClick, string hoverText)
    {
    ...
    private void AddNextButton(UIPanel playersPanel, float playerPanelPadding, float navButtonWidth, float cardHeight, float scale, Action onClick, string hoverText)
    {
