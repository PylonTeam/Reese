...
internal static class SpectatorModeNetHandler
{
    ...
    internal enum SetModeReason : byte
    {
        Default,
        JoinPanelChoice
    }
    ...
    public static void SendRequestSetMode(int slot, SpectateMode mode, SetModeReason reason = SetModeReason.Default)
    {
    ...
    private static void ReceiveSetMode(BinaryReader reader, int sender)
    {
        ...
        SetModeReason reason = (SetModeReason)reader.ReadByte();

        if (slot < 0 || slot >= Main.maxPlayers || !Main.player[slot].active)
            return;

        if (!SpectatorModeSystem.TryAcceptClientModeRequest(sender, slot, mode, reason, out string errorMessage))
        {
            if (!string.IsNullOrWhiteSpace(errorMessage))
                Log.Warn($"Spectator mode request denied for sender {sender}, slot {slot}: {errorMessage}");

            SpectatorModeNetHandler.SendSyncModes(sender);
            return;
        }
    ...
    private static ModPacket CreatePacket(SpectatorPacket packetType)
    {
        ...
        packet.Write((byte)ReesePacketType.RequestToggleSpectateMode);
