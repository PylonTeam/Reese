...
internal sealed class DrawNameplatesSpectator : ModSystem
{
    ...
    private static bool TryGetDistance(int screenWidth, int screenHeight, Vector2 screenPosition, Player localPlayer, DynamicSpriteFont font, Player otherPlayer, string name, out Vector2 namePlatePos, out float namePlateDist, out Vector2 measurement)
    {
        ...
        object[] args = [screenWidth, screenHeight, screenPosition, localPlayer, font, otherPlayer, name, namePlatePos, namePlateDist, measurement];
        getDistanceMethod.Invoke(null, args);

        namePlatePos = (Vector2)args[7];
        namePlateDist = (float)args[8];
        measurement = (Vector2)args[9];
    ...
    public void VanillaDraw()
    {
        ...
        for (int i = 0; i < 255; i++)
        {
            ...
            if (!TryGetDistance(screenWidth, screenHeight, screenPosition, player2, value, player3, name, out var namePlatePos, out var namePlateDist, out var measurement))
                continue;
    ...
    private static bool ShouldDrawNamePlate(Player localPlayer, Player otherPlayer)
    {
        ...
        return SpectatorModeSystem.IsInSpectateMode(localPlayer) ? true : localPlayer.team != 0 && otherPlayer.team == localPlayer.team;
    }
}
