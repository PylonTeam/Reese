...
/// </summary>
internal class PlaybackEnterWorldGhost : ModSystem
{
    ...
    void ILHook2(ILContext il)
    {
        ...
        cur.EmitDelegate<Action<BinaryReader>>(i =>
        {
            ...
            Player JoinedPlayer = Main.ActivePlayerFileData?.Player?.SerializedClone();

            if (JoinedPlayer == null)
            {
                Log.Chat("Failed to capture joined player data.");
                return;
            }
            Log.Chat($"Intercepted joined player with game mode={game_mode}, player name={JoinedPlayer?.name} ({JoinedPlayer?.whoAmI})");
    ...
    {
        Log.Debug("Cleared JoinPlayerSnapshot on world unload");
    }
}
