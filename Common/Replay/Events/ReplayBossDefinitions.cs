...
internal readonly record struct ReplayBossDefinition(string Key, string Name, int HeadNpcId, int[] NpcIds, Func<bool> IsDefeated)
{
    ...
    public TimelineEvent CreateSummonedEvent(uint tick)
    {
        return new TimelineEvent(tick, TimelineEventCategory.BossSummoned, Key, Loc.Get("ReplayHud.Events.BossSummoned", Name), TimelineEventIconKind.BossHead, HeadNpcId);
    }

    public TimelineEvent CreateDefeatedEvent(uint tick)
    {
        return new TimelineEvent(tick, TimelineEventCategory.BossDefeated, Key, Loc.Get("ReplayHud.Events.BossDefeated", Name), TimelineEventIconKind.BossHead, HeadNpcId);
    }
}
