...
public enum TimelineEventCategory : byte
{
    BossDefeated = 1,
    PlayerDeath = 2,
    InvasionStarted = 3,
...
public enum TimelineEventIconKind : byte
{
    None = 0,
    BossHead = 1,
    MapDeath = 2,
...
public readonly record struct TimelineEvent(
    ...
    string Key,
    string Text,
    TimelineEventIconKind IconKind = TimelineEventIconKind.None,
    int IconId = 0,
    TimelinePlayerHeadSnapshot? PlayerHead = null);

public readonly record struct TimelinePlayerHeadSnapshot(
    int PlayerIndex,
    int Team,
    int Hair,
    int HairDye,
    ...
    uint ShoeColor)
{
    public static TimelinePlayerHeadSnapshot FromPlayer(Player player)
    {
        return new TimelinePlayerHeadSnapshot(
            player?.whoAmI ?? -1,
            player?.team ?? 0,
            player?.hair ?? 0,
            player?.hairDye ?? 0,
