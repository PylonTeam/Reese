...
namespace Reese.Common.Replay.Events;

internal static class TimelineRecorder
{
    private static readonly List<TimelineEvent> events = [];
    ...
    public static TimelineEvent[] Finish()
    {
        active = false;
    ...
    public static void Add(TimelineEvent timelineEvent)
    {
        if (!active)
            return;
    ...
    public static void RecordPlayerDeath(Player player, PlayerDeathReason damageSource, uint tick)
    {
        ...
        Add(new TimelineEvent(tick, TimelineEventCategory.PlayerDeath, $"player-death:{player.whoAmI}:{tick}", Loc.Get("ReplayHud.Events.PlayerDied", playerName), TimelineEventIconKind.MapDeath));
    }
    ...
    private static bool TryRecordPlayerKill(Player killedPlayer, PlayerDeathReason damageSource, uint tick)
    {
        ...
        Add(new TimelineEvent(
            tick,
            TimelineEventCategory.PlayerKill,
            $"player-kill:{killerIndex}:{killedPlayer.whoAmI}:{tick}",
            Loc.Get("ReplayHud.Events.PlayerKilled", killerName, killedName),
            TimelineEventIconKind.PlayerHead,
            weaponItemId,
            TimelinePlayerHeadSnapshot.FromPlayer(killerPlayer)));
    ...
    public static void RecordPlayerJoined(Player player, uint tick)
    {
        ...
        Add(new TimelineEvent(
            tick,
            TimelineEventCategory.PlayerJoined,
            $"player-join:{player.whoAmI}:{tick}",
            Loc.Get("ReplayHud.Events.PlayerJoined", playerName),
            TimelineEventIconKind.PlayerHead,
            player.whoAmI,
            TimelinePlayerHeadSnapshot.FromPlayer(player)));
    }
    ...
    public static void RecordPlayerLeft(Player player, uint tick)
    {
        ...
        Add(new TimelineEvent(
            tick,
            TimelineEventCategory.PlayerLeft,
