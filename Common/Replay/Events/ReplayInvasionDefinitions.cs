...
{
    public static TimelineEvent CreateEvent(uint tick, int invasionType)
    {
        string name = GetName(invasionType);
        int itemId = GetIconItemId(invasionType);
        TimelineEventIconKind iconKind = itemId > 0 ? TimelineEventIconKind.Item : TimelineEventIconKind.None;

        return new TimelineEvent(tick, TimelineEventCategory.InvasionStarted, $"invasion:{invasionType}", Loc.Get("ReplayHud.Events.InvasionStarted", name), iconKind, itemId);
    }
