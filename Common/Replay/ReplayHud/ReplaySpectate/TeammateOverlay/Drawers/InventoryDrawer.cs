...
namespace Reese.Common.Replay.ReplayHud.ReplaySpectate.TeammateOverlay.Drawers;

public static class InventoryDrawer
{
    private const BindingFlags StaticAccess = BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Static;
    private const BindingFlags InstanceAccess = BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance;

    private static readonly FieldInfo MainMHField = typeof(Main).GetField("mH", StaticAccess);
    private static readonly PropertyInfo MainMHProperty = typeof(Main).GetProperty("mH", StaticAccess);
    private static readonly MethodInfo MainDrawPageIconsMethod = typeof(Main).GetMethod("DrawPageIcons", StaticAccess, null, [typeof(int)], null);
    private static readonly FieldInfo MainSettingsButtonIsPushedToSideField = typeof(Main).GetField("_settingsButtonIsPushedToSide", StaticAccess);
    private static readonly PropertyInfo MainSettingsButtonIsPushedToSideProperty = typeof(Main).GetProperty("_settingsButtonIsPushedToSide", StaticAccess);
    private static readonly MethodInfo MainDrawDefenseCounterMethod = typeof(Main).GetMethod("DrawDefenseCounter", StaticAccess, null, [typeof(int), typeof(int)], null);
    private static readonly FieldInfo MainAchievementAdvisorField = typeof(Main).GetField("_achievementAdvisor", InstanceAccess);
    private static readonly PropertyInfo MainAchievementAdvisorProperty = typeof(Main).GetProperty("_achievementAdvisor", InstanceAccess);
    private static readonly FieldInfo ModAccessorySlotPlayerExAccessorySlotField = typeof(ModAccessorySlotPlayer).GetField("exAccessorySlot", InstanceAccess);
    private static readonly PropertyInfo ModAccessorySlotPlayerExAccessorySlotProperty = typeof(ModAccessorySlotPlayer).GetProperty("exAccessorySlot", InstanceAccess);
    private static readonly FieldInfo ModAccessorySlotPlayerExDyesAccessoryField = typeof(ModAccessorySlotPlayer).GetField("exDyesAccessory", InstanceAccess);
    private static readonly PropertyInfo ModAccessorySlotPlayerExDyesAccessoryProperty = typeof(ModAccessorySlotPlayer).GetProperty("exDyesAccessory", InstanceAccess);
    ...
    private static int MainMH => GetStaticInt(MainMHField, MainMHProperty);

    private static int DrawMainPageIcons(int y)
    {
        return MainDrawPageIconsMethod?.Invoke(null, [y]) is int page ? page : -1;
    }

    private static bool SettingsButtonIsPushedToSide => GetStaticBool(MainSettingsButtonIsPushedToSideField, MainSettingsButtonIsPushedToSideProperty);

    private static void DrawMainDefenseCounter(int x, int y)
    {
        MainDrawDefenseCounterMethod?.Invoke(null, [x, y]);
    }

    private static object GetAchievementAdvisor()
    {
        if (Main.instance == null)
            return null;

        return MainAchievementAdvisorField?.GetValue(Main.instance) ?? MainAchievementAdvisorProperty?.GetValue(Main.instance);
    }

    private static bool AchievementAdvisorCanDrawAboveCoins(object achievementAdvisor)
    {
        return GetInstanceBool(achievementAdvisor, "CanDrawAboveCoins", true);
    }

    private static void DrawOneAchievement(object achievementAdvisor, SpriteBatch spriteBatch, Vector2 position, bool large)
    {
        MethodInfo method = achievementAdvisor.GetType().GetMethod("DrawOneAchievement", InstanceAccess, null, [typeof(SpriteBatch), typeof(Vector2), typeof(bool)], null);
        method ??= FindInstanceMethodByNameAndParameterCount(achievementAdvisor.GetType(), "DrawOneAchievement", 3);
        method?.Invoke(achievementAdvisor, [spriteBatch, position, large]);
    }

    private static Item[] GetExAccessorySlot(ModAccessorySlotPlayer modPlayer)
    {
        return GetItemArray(modPlayer, ModAccessorySlotPlayerExAccessorySlotField, ModAccessorySlotPlayerExAccessorySlotProperty);
    }

    private static Item[] GetExDyesAccessory(ModAccessorySlotPlayer modPlayer)
    {
        return GetItemArray(modPlayer, ModAccessorySlotPlayerExDyesAccessoryField, ModAccessorySlotPlayerExDyesAccessoryProperty);
    }

    private static int GetStaticInt(FieldInfo field, PropertyInfo property, int fallback = 0)
    {
    ...
    private static bool GetStaticBool(FieldInfo field, PropertyInfo property, bool fallback = false)
    {
    ...
    private static bool GetInstanceBool(object source, string name, bool fallback = false)
    {
        if (source == null)
            return fallback;
        ...
        if (type.GetField(name, InstanceAccess)?.GetValue(source) is bool fieldValue)
            return fieldValue;

        if (type.GetProperty(name, InstanceAccess)?.GetValue(source) is bool propertyValue)
            return propertyValue;

        return fallback;
    }

    private static Item[] GetItemArray(object source, FieldInfo field, PropertyInfo property)
    {
        if (field?.GetValue(source) is Item[] fieldValue)
            return fieldValue;

        if (property?.GetValue(source) is Item[] propertyValue)
            return propertyValue;

        return null;
    }

    private static MethodInfo FindInstanceMethodByNameAndParameterCount(Type type, string name, int parameterCount)
    {
        foreach (MethodInfo method in type.GetMethods(InstanceAccess))
        {
    ...
    {
        int num20 = 174 + MainMH;
        int num22 = DrawMainPageIcons(num20 - 32);
        if (num22 > -1)
        {
            string hoverText = "";
            switch (num22)
    ...
    public static void DrawArmor(Player player)
    {
        ...
        try
        {
            ...
            if (Main.EquipPage == 0)
            {
                ...
                if (SettingsButtonIsPushedToSide)
                {
                    num37--;
                }
                ...
                DrawMainDefenseCounter((int)defPos.X, (int)defPos.Y);

                DrawDefenseCounter(player);

                object achievementAdvisor = GetAchievementAdvisor();
                if (achievementAdvisor != null && !AchievementAdvisorCanDrawAboveCoins(achievementAdvisor))
                {
                    Vector2 achievePos = new Vector2(defPos.X - 10f - 47f - 47f - 14f - 14f, defPos.Y - 56f * Main.inventoryScale * 0.5f);
                    DrawOneAchievement(achievementAdvisor, Main.spriteBatch, achievePos, false);
    ...
    public static void DrawAccSlots(Player player)
    {
        ...
        try
        {
            ...
            Item[] exAccessorySlot = GetExAccessorySlot(modPlayer);
            Item[] exDyesAccessory = GetExDyesAccessory(modPlayer);
            ...
            for (int slot = 0; slot < modPlayer.SlotCount; slot++)
            {
                ...
                Item[][] inventories = [exAccessorySlot, exAccessorySlot, exDyesAccessory];
                int[] contexts = [10, 11, 12];
    ...
    private static void DrawHeldItemSlot(Player player, int x, int y)
    {
        ...
        ItemSlot.Draw(Main.spriteBatch, heldItemSlot, ItemSlot.Context.InventoryItem, 0, new Vector2(x, y));
    }
