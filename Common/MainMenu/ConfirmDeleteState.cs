...
public class ConfirmDeleteState : UIState
{
    public string TargetName;
    public Action OnYes;
    public Action OnNo;
    public UIState NextYesState;
    public UIState NextNoState;

    private const int Capacity = 20;
    private readonly string[] _labels = new string[Capacity];
    private readonly bool[] _isHeader = new bool[Capacity];
    private readonly bool[] _isDisabled = new bool[Capacity];
    private readonly bool[] _isDimmed = new bool[Capacity];
    private readonly int[] _yOffset = new int[Capacity];
    private readonly int[] _xOffset = new int[Capacity];
    private readonly byte[] _colorType = new byte[Capacity];
    private readonly float[] _scale = new float[Capacity];
    private readonly bool[] _leftAligned = new bool[Capacity];
    private readonly bool[] _menuWide = new bool[Capacity];
    private readonly float[] _scaleAnim = new float[Capacity];
    ...
    protected override void DrawSelf(SpriteBatch spriteBatch)
    {
        ...
        {
            _labels[i] = null;
            _isHeader[i] = _isDisabled[i] = _isDimmed[i] = _leftAligned[i] = false;
            _yOffset[i] = _xOffset[i] = 0;
            _colorType[i] = 0;
            _scale[i] = 1f;
        }
        ...
        _isHeader[0] = true;
        _labels[1] = Lang.menu[104].Value; // Yes
        _labels[2] = Lang.menu[105].Value; // No
        int itemCount = 3;

        int menuTop = 250;
        int menuSpacing = 80;
        int menuCenterX = Main.screenWidth / 2;
        ...
        DrawVanillaMenuLoop(
            ...
            Main.netMode,
            Main.mouseTextColor,
            Main.ColorOfTheSkies, // use same base color as main menu
            Main.mcColor,
            Main.hcColor,
            Main.highVersionColor,
            Main.errorColor,
            ref _focusedIndex,
            ref _clickedIndex,
            ref _rightClickedIndex,
            prevFocused,
            _menuWide,
            _scaleAnim,
            Main.mouseX,
        ...
        {
            OnYes?.Invoke();
            if (NextYesState != null) Main.MenuUI.SetState(NextYesState);
        }
        else if (_clickedIndex == 2 || backPressed)
        {
            OnNo?.Invoke();
            if (NextNoState != null) Main.MenuUI.SetState(NextNoState);
        }
    }
